...
export function EventLog({ events, fullPage = false, limit }: { events: Activity[]; fullPage?: boolean; limit?: number }) {
  ...
  const displayedEvents = limit ? visibleEvents.slice(0, limit) : visibleEvents
  return <section className={`event-log ${fullPage ? 'full-page' : ''}`}><div className="event-heading"><div><p className="eyebrow">RUNTIME ACTIVITY</p><h2>{fullPage ? 'Event history' : 'Live operations timeline'}</h2></div><span className="retention-note">IN MEMORY · {events.length} EVENTS</span></div>{fullPage ? <div className="activity-toolbar"><div className="activity-filters">{(['all', 'alarm', 'detector', 'stream'] as const).map((option) => <button key={option} className={filter === option ? 'active' : ''} onClick={() => setFilter(option)}>{option === 'all' ? 'All events' : option}</button>)}</div><label><span className="eyebrow">SEARCH LOG</span><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Asset, event, or decision" /></label></div> : null}<div className="events-table"><div className="table-labels"><span>TIME</span><span>SOURCE</span><span>EVENT</span><span>DETAIL</span><span>STATUS</span></div>{displayedEvents.length === 0 ? <div className="empty-row">No matching runtime activity.</div> : displayedEvents.map((event, index) => { const item = eventDetail(event); return <div className="event-row" key={`${event.recorded_at}-${event.event}-${index}`}><time>{new Date(event.recorded_at * 1000).toLocaleTimeString()}</time><span><b>{item.source}</b></span><span><b>{event.event}</b></span><span>{item.detail}</span><span className={`event-severity ${item.label.toLowerCase()}`}>{item.label}</span></div> })}</div></section>
}
